...
namespace acc3d
{
	namespace Core
	{
		...
		{
			m_Window = SDL_CreateWindow(title.data(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
			if (!m_Window)
			{
				acc3d_error("Something has gone wrong while creating the SDL window. Error Message:\n {0}", SDL_GetError());
				return;
			}
		}

		Window::Window(uint32_t w, uint32_t h, std::string_view title, uint32_t flags)
		{
			m_Window = SDL_CreateWindow(title.data(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, flags);
			if (!m_Window)
			{
				acc3d_error("Something has gone wrong while creating the SDL window. Error Message:\n {0}", SDL_GetError());
				return;
